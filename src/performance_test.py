...
def create_random_image(max_size_mb=5):
    ...
    logger.info(
        f"Generated random image: {width}x{height} pixels, "
        f"{size_mb:.2f} MB"
    )
...
def generate_sas_url(blob_service_client, container_name, blob_name):
    ...
    # for SAS generation when using managed identity, such as user
    # delegation SAS
    try:
        ...
        if account_key:
            ...
            # For managed identity scenarios, we'll just use the blob URL
            # without SAS In a real scenario, you would generate a user
            # delegation SAS
            logger.warning(
            ...
            logger.info(
                f"Blob URL generation completed in "
...
def download_blob_via_sas(sas_url, blob_service_client=None,
    ...
    try:
        ...
        # If URL download fails, try using blob client directly
        # (for auth scenarios)
        logger.warning(
            f"URL download failed: {e}. Trying direct blob client access."
        )
...
def main():
    ...
    logger.info(
        f"Starting performance test against storage account: "
    ...
    try:
        ...
        sas_url, sas_generation_time_ms = generate_sas_url(
        ...
        upload_to_download_time_ms = (
            download_end_time - upload_start_time
        ) * 1000

        metrics.metrics['total_time_ms'] = round(total_time_ms, 2)
        metrics.metrics['upload_to_download_time_ms'] = round(
            upload_to_download_time_ms, 2
        )

        # Verify download
        if len(downloaded_data) == len(image_data):
            logger.info(
                "✓ Download verification successful - file sizes match"
            )
        else:
            logger.warning(
                f"⚠ Download verification failed - size mismatch: "
                f"{len(downloaded_data)} vs {len(image_data)}"
            )
        ...
        logger.info(
            f"SAS Generation Time: "
            f"{metrics.metrics['sas_generation_time_ms']} ms"
        )
